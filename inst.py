...
def ADC():
	glob.ALU = glob.DB
	if glob.ALU + glob.A >= 0x100: 		# if overflowing,
...
def ADCIY(): #Add with Carry indirect indexed, 5+1Cycs, $71 *CR*
	if glob.Cyc == 5:
		...
		glob.ADH = glob.DB
		print(glob.ADL+glob.Y, "NIGGGGGAAA")
		if (glob.ADL+glob.Y)<=0xFF: glob.ADL=Add(glob.Y); glob.Cyc -=1
		else: glob.ADL = Add(glob.Y); glob.ALU = glob.ADH 
		AHL()
	elif glob.Cyc == 2:
...
def SBC():
	glob.ALU = glob.DB
	if glob.A+(256-glob.ALU)+~(glob.P&0b1) >=0x100:
		glob.np = 1
	else:
		glob.np = 0
	glob.ALU = 256-glob.ALU
	C = (((glob.ALU&0x7F)+(glob.A&0x7F))&0x80)>>7
	M = (glob.ALU&0b10000000)>>7
	N = (glob.A&0b10000000)>>7
	glob.vv = ((M&N)^C)&~(M^N)
...
def SBCIM(): #Sub with Carry Immediate, 2Cyc, $E9 **
	if glob.Cyc == 1:
		SBC()


INSCOD = [0x0,0x1,0x2,0x3,0x4,0x5,0x6,0x7,0x8,0x9,0xA,0xB,0xC,0xD,0xE,0xF, #0x0
			BPLRL,0x1,0x2,0x3,0x4,0x5,0x6,0x7,CLCIP,0x9,0xA,0xB,0xC,0xD,0xE,0xF,
			0x0,0x1,0x2,0x3,0x4,0x5,0x6,0x7,0x8,0x9,0xA,0xB,0xC,0xD,0xE,0xF, #0x2
			BMIRL,0x1,0x2,0x3,0x4,0x5,0x6,0x7,SECIP,0x9,0xA,0xB,0xC,0xD,0xE,0xF,
			0x0,0x1,0x2,0x3,0x4,0x5,0x6,0x7,0x8,0x9,0xA,0xB,0xC,0xD,0xE,0xF, #0x4
			BVCRL,0x1,0x2,0x3,0x4,0x5,0x6,0x7,CLIIP,0x9,0xA,0xB,0xC,0xD,0xE,0xF,
			...
			0x0,0x1,0x2,0x3,0x4,0x5,INCZP,0x7,INXIP,SBCIM,NOPIP,0xB,0xC,0xD,INCAB,0xF, #0xE
			BEQRL,0x1,0x2,0x3,0x4,0x5,INCZX,0x7,SEDIP,0x9,0xA,0xB,0xC,0xD,INCAX,0xF,]

INSCYC = [0x0,0x1,0x2,0x3,0x4,0x5,0x6,0x7,0x8,0x9,0xA,0xB,0xC,0xD,0xE,0xF, #0x0
			4,0x1,0x2,0x3,0x4,0x5,0x6,0x7,2,0x9,0xA,0xB,0xC,0xD,0xE,0xF,
			0x0,0x1,0x2,0x3,0x4,0x5,0x6,0x7,0x8,0x9,0xA,0xB,0xC,0xD,0xE,0xF, #0x2
			4,0x1,0x2,0x3,0x4,0x5,0x6,0x7,2,0x9,0xA,0xB,0xC,0xD,0xE,0xF,
			0x0,0x1,0x2,0x3,0x4,0x5,0x6,0x7,0x8,0x9,0xA,0xB,0xC,0xD,0xE,0xF, #0x4
			4,0x1,0x2,0x3,0x4,0x5,0x6,0x7,2,0x9,0xA,0xB,0xC,0xD,0xE,0xF,
			...
			0x0,0x1,0x2,0x3,0x4,0x5,5,0x7,2,2,2,0xB,0xC,0xD,6,0xF, #0xE
			4,0x1,0x2,0x3,0x4,0x5,6,0x7,2,0x9,0xA,0xB,0xC,0xD,7,0xF,]
